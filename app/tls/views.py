...
def showex(uuid="uuid-39b00dce-3e66-4507-834f-1ec3eb135b29"):
    ...
    if uuid.startswith("CONCEPT"):
        ## we need to look for all words in this concept, then loop
        ## through and find the examples
        concept_id = uuid.split("-", 1)[-1]
        w = tlsdb.hgetall(con_key + concept_id)
        if w.has_key('words'):
            wrds=eval(w['words'])
            for nx, sw in enumerate(wrds):
                swc = sw['head'].split()[0]
                synw = sw['synwords']
                for synwi in synw:
                    if synwi.has_key('CUSTOM_ID'):
                        swid = synwi['CUSTOM_ID']
                        lnk = "<a href='#%s'>(%d)</a>" % (sw['CUSTOM_ID'], nx +1)
                        l = tlsdb.lrange(swl_key + swid, 0, -1)
                        for b in [a.split("##") for a in l]:
                            lnx = re.sub("(%s)" % (swc), "<span style='color:blue;'>\\1</span>", b[5])
                            txtloc = b[0].split(":")
